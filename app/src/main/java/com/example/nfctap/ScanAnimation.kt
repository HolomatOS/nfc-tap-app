...
fun NfcScanScreen(scanTrigger: Int, onVibrate: (Long) -> Unit) {
    ...
    LaunchedEffect(scanTrigger) {
        ...
        onVibrate(60)
        convergeProgress.animateTo(1f, tween(550, easing = FastOutSlowInEasing))
        onVibrate(120)
    ...
    Canvas(modifier = Modifier.fillMaxSize().background(Color.Black)) {
        ...
        particles.forEach { p ->
            ...
            drawCircle(color = Color.White.copy(alpha = alpha), radius = p.size, center = Offset(drawX, drawY))
        }

        drawCircle(color = Color.White.copy(alpha = 0.9f), radius = circleRadius * circleScale.value, center = center, style = Stroke(width = 4f))
        ...
        if (checkProgress.value > 0f) {
            ...
            val path = Path().apply {
                ...
                lineTo(p1.x + (p2.x - p1.x) * firstLeg, p1.y + (p2.y - p1.y) * firstLeg)
                if (checkProgress.value > 0.45f) {
                    val secondLeg = ((checkProgress.value - 0.45f) / 0.55f).coerceIn(0f, 1f)
                    lineTo(p2.x + (p3.x - p2.x) * secondLeg, p2.y + (p3.y - p2.y) * secondLeg)
                }
            }
            drawPath(path = path, color = Color.White, style = Stroke(width = 8f, cap = StrokeCap.Round))
        }
    }
}
